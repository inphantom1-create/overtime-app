...
function saveMembers(list) {
  try {
    localStorage.setItem('overtime_members', JSON.stringify(list));
  } catch(e) {}
}

/** 신청 폼의 이름 드롭다운 다시 채우기 */
...
function addMember() {
  ...
  if (members.includes(name)) {
    errEl.textContent = '이미 등록된 이름입니다.';
    return;
  }
...
function validateForm() {
  ...
  if (!name) {
    setError('err-name', '이름을 선택해 주세요.', 'name'); valid = false;
  } else { clearError('err-name', 'name'); }

  if (!job) {
    setError('err-job', '직무를 선택해 주세요.', 'job'); valid = false;
  } else { clearError('err-job', 'job'); }

  if (!workDate) {
    setError('err-date', '근무 날짜를 선택해 주세요.', 'work-date'); valid = false;
  ...
  if (!reason) {
    setError('err-reason', '근무 사유를 입력해 주세요.', 'reason'); valid = false;
...
async function submitForm() {
  ...
  const payload = {
    ...
    job:       document.getElementById('job').value,       // 직무
...
function adminLogin() {
  ...
  if (pw === ADMIN_PASSWORD) {
    ...
    showPage('admin');
    refreshNameSelect(); // 관리자 필터 이름 목록 동기화
...
function openDeleteModal(idx) {
  ...
  document.getElementById('modal-desc').textContent =
    `${row.name} · ${row.workDate} · ${row.overtime}\n이 내역을 삭제할까요?`;
...
async function confirmDelete() {
  ...
  try {
    ...
    showToast('🗑️ 삭제 요청을 보냈습니다. 잠시 후 다시 조회해주세요.', '');
    // 화면에서도 즉시 제거
